...
class CardMgr(BaseRepository):
    ...
    #     self.all_cards_to_block = await card_repository.get_cards_by_filters(balance_ids=balance_ids_to_block_cards)
    ...
    #     self.all_cards_to_block = await card_repository.get_cards_by_filters(balance_ids=balance_ids_to_activate_cards)

    # async def process_khnp(self) -> None:
    #     # Получаем статусы карт из системы ХНП
    #     khnp_connector = KHNPController(self.session, self.logger)
