...
class KHNPParser:
    ...
    def __init__(self):
        self.logger = get_logger(name="KHNPParser", filename="celery.log")
        self.site = KHNP_URL
    ...
    def get_card_state(self, card_num: str) -> CardStatus:
        if not self.cards:
    ...
    def set_card_state(self, card_num: str, activate: bool) -> None:
        if 'info.html' not in self.driver.current_url:
            self.open_cards_page()
        ...
        if card_status != CardStatus.UNKNOWN:
            if (activate and card_status in [CardStatus.BLOCKED, CardStatus.BLOCKING_PENDING]) or \
                    (not activate and card_status in [CardStatus.ACTIVE, CardStatus.ACTIVATE_PENDING]):
