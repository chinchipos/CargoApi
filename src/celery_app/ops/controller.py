...
class OpsController(BaseRepository):
    ...
    async def load_transactions(self) -> None:
        ...
        self.logger.info(f'Новые транзакции от системы ОПС: {len(remote_transactions)} шт')
    ...
    async def process_new_remote_transaction(self, remote_transaction: Dict[str, Any]) \
        ...
        transaction_data = await self.helper.process_new_remote_transaction(
            ...
            comments='',
            system_id=self.system.id,
            transaction_external_id=str(remote_transaction["transactionID"]),
            transaction_time=remote_transaction['transactionDateTime'],
            transaction_sum=remote_transaction['amountWithoutDiscountRounded'],
            transaction_fuel_volume=remote_transaction['quantity'],
            transaction_price=remote_transaction['priceWithoutDiscount']
        )
